...
def get_chat_response(query):
    ...
    # Fix typos in query
    q = query.lower().replace("hoe", "how").replace("heats", "heat").replace("intheir", "in their").replace("reseacrh", "research").replace("nexy", "next").replace("lebron's", "lebron").replace("what was", "research").replace("hte", "the").replace("win", "beat")
    teams_mentioned = [full_name for alias, full_name in TEAM_ALIASES.items() if alias in q]
    team = next((t for t in teams_mentioned if t in q.split("beat")[0] or "when" in q or "research" in q or "tell" in q or "what" in q or "how" in q), teams_mentioned[0] if teams_mentioned else None)
    ...
    if team:
        ...
        season = f"49-31 as of Apr 9. Solid!" if "lakers" in q and "won" in q and "season" in q else "record TBD—stay tuned!"
        ...
        # Build response
        response = f"{response}{season if 'won' in q and 'season' in q else last_score if 'last' in q or 'score' in q else next_odds} {date if 'last' not in q and 'score' not in q and 'won' not in q and 'season' not in q and not ('lakers' in q and 'rockets' in q and 'beat' in q) and not ('knicks' in q and 'pistons' in q and 'beat' in q) else ''}\nNext: Stats? Odds?"
